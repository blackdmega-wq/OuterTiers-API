...
/** Remove only legacy player snapshots keyed by internal serial ids. */
router.post("/admin/repair-legacy-player-ids", async (req, res) => {
  if (!requireSecret(req, res)) return;
  try {
    const legacy = await db.select({ id: playersTable.id, userId: playersTable.userId, username: playersTable.username }).from(playersTable).where(sql`NOT (${playersTable.userId} ~ '^[0-9]{17,20}
...
export default router;
    ...
    if (legacy.length) await db.delete(playersTable).where(sql`NOT (${playersTable.userId} ~ '^[0-9]{17,20}
router.post("/admin/sync", async (req, res) => {
  if (!requireSecret(req, res)) return;
  const result = await syncAllPlayers();
  return res.json({ ok: true, ...result });
...
export default router;
    ...
    return res.json({ ok: true, deleted: legacy.length, sample: legacy.slice(0, 20) });
  } catch (err) {
    console.error("[/api/admin/repair-legacy-player-ids] DB error:", (err as Error).message);
    return res.status(503).json({ error: "Database temporarily unavailable. Please try again." });
  }
});
